(() => {
  ...
  H.requireAuth(); H.fillIdentity(); H.bindLogout();
  ...
  form.addEventListener("submit", async event => {
    ...
    if(!current || !next || !confirm){ message.textContent = "Nhập đầy đủ ba ô mật khẩu."; return; }
    if(next.length < 6){ message.textContent = "Mật khẩu mới phải có ít nhất 6 ký tự."; return; }
    if(next !== confirm){ message.textContent = "Mật khẩu nhập lại chưa khớp."; return; }
