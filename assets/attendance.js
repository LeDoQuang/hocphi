  ...
  H.api("/api/attendance").then(data => {
    ...
    for (const [key, group] of groups) {
      ...
      for (const item of group) {
        ...
        left.append(H.el("span", "meta", [H.formatDate(item.date), item.note || null].filter(Boolean).join(" · ")));
    ...
    document.getElementById("attendanceList").replaceChildren(H.el("div", "empty", error.message));
