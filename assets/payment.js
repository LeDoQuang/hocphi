(() => {
  ...
  function renderCharges(charges) {
    ...
    for (const charge of charges) {
      ...
      if (Number(charge.remaining) !== Number(charge.amount)) {
        right.append(H.el("span", "meta", `Khoản gốc ${H.formatVnd(charge.amount)}`));
      }
  ...
  H.api("/api/me").then(data => {
    ...
    document.getElementById("chargesList").replaceChildren(H.el("div", "empty", error.message));
